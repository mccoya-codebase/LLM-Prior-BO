"""
Generate Prior Sets via per-dimension scoring with Temperature Ablation.
Toggle between Gemini 2.5 Pro, Gemini 2.5 Flash-Lite, and Llama 3.3.
Evaluates explicit temperature settings (0.3, 0.5) to test LLM generation variance.
Specifically restricted to N=10 and isolates outputs into a dedicated '/temp_test' directory.
"""
...
def fetch_and_save_scores(llm_client, mappings: dict, predicted_ranks_dir: Path, param_columns: list, reaction_name: str, paper_text: str = None, temperature: float = 0.8) -> dict:
    value_to_score = {}
    ...
    for col in param_columns:
        ...
        # Temperature dynamically injected here
        response = llm_client.generate_completion(prompt=prompt, temperature=temperature)
...
def main():
    ...
    N_PRIORS = [10]             # Restricted strictly to N=10 for this test
    N_SETS = 30
    SKIP_SCORING = False       
    # ---------------------------------------------------------
    ...
    # Base folder isolated to temp_test
    test_out_base = _script_dir / "temp_test"
    ...
    try:
        if not SKIP_SCORING:
            ...
            # Create the cache exactly once to save time across multiple temperature runs
            llm_client.create_cache(paper_text)

        for temp in TEMPERATURES:
            print(f"\n====================================================================")
            print(f"--- Generating sets for {MODEL_CHOICE} | Temp = {temp} ({DATASET_NAME}) ---")
            print(f"====================================================================")
            
            # Tag the model dir with the specific temperature
            model_dir = f"{model_info['dir_name']}_temp_{temp}"
            predicted_ranks_base = test_out_base / "predicted_ranks" / model_dir / DATASET_NAME
            reference_out_base = test_out_base / "priors" / DATASET_NAME / model_dir / f"n_priors_{N_PRIORS[0]}"
            ...
            for current_index in range(start_index, end_index):
                ...
                if SKIP_SCORING:
                    ...
                    value_to_score = fetch_and_save_scores(
                        ...
                        mappings, 
                        predicted_ranks_dir, 
                        config["param_columns"], 
                        config["reaction_name"], 
                        paper_text,
                        temperature=temp # Dynamically injected
                    )
                ...
                for n_priors in N_PRIORS:
                    ...
                    out_base = test_out_base / "priors" / DATASET_NAME / model_dir / f"n_priors_{n_priors}"
                    out_base.mkdir(parents=True, exist_ok=True)
                    
                    out_path = out_base / f"set_{current_index}.csv"
                    df_yield_mapped.to_csv(out_path, index=False)
                    
            print(f"All {DATASET_NAME} prior sets for {MODEL_CHOICE} (Temp={temp}) generated successfully!")

    finally:
        # Clean up the cache when all temperatures are completely finished
