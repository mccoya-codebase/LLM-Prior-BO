"""
Run Prior-guided Bayesian Optimization (PiBO) for Gemini 2.5 Flash-Lite priors.
...
# ---------------------------------------------------------
# 1. Get the directory of the current script (...\1_LLM_Prior_BO)
...
def main():
    ...
    # Updated to run the Gemini 2.5 Flash-Lite priors
    LLM_MODELS = ["llama_3_3"]
    
    N_PRIORS = [1, 3, 5, 10]      # Run all batch sizes
    MAX_SETS = 30                 # Number of prior sets (set_0 to set_29)
    BETAS = [1]                   # PiBO decay weights (1=soft)
    ...
    for llm_model in LLM_MODELS:
        print(f"\n=======================================================")
        print(f" STARTING PiBO CAMPAIGN: {DATASET_NAME} | {llm_model.upper()}")
        print(f"=======================================================\n")
        
        for set_num in range(MAX_SETS):
            print(f"=== Evaluating Set {set_num} ({llm_model.upper()}) ===")
            
            for current_n in N_PRIORS:
                # Dynamically route the prior path based on DATASET_NAME and LLM_MODEL
                prior_path = _script_dir / "priors" / DATASET_NAME / llm_model / f"n_priors_{current_n}" / f"set_{set_num}.csv"
                ...
                # Output to a specific directory within piBO_runs, grouped by dataset and model
                out_base = _script_dir / "piBO_runs" / DATASET_NAME / llm_model / f"n_priors_{current_n}" / f"set_{set_num}"
